    ...
    if len(subscribed_path.poses) != 0 and not status.data:
        ...
        if next_point_distance < 0.4:
            ...
            if wp_index >= len(subscribed_path.poses):
                ...
                status.data = True #Updating the status to indicate that the robot has achieved its goal
                #Reseting the variables value
                wp_index = 1
                pre_beta = 0
                pre_cte = 0
                pre_time = 0
        ...
        if (not status.data): #Only proceding if the robot hasn't achieved its goal yet
...
if __name__ == '__main__':
    ...
    try:
        while not rospy.is_shutdown():
            move_status_pub.publish(status) #Updating the move_status topic
